...
# Determinación robusta de rutas tanto en modo desarrollo como congelado (.exe con PyInstaller)
if getattr(sys, 'frozen', False):
    # Si la aplicación se ejecuta compilada como ejecutable (.exe)
    RAIZ_PROYECTO = os.path.dirname(sys.executable)
    CARPETA_SRC = getattr(sys, '_MEIPASS', RAIZ_PROYECTO)
else:
    # Si se ejecuta como script en src/view/gui/ (tres niveles por debajo de la raíz: gui -> view -> src -> raíz)
...
class PensionApp(App):
    ...
    def exportar_historial_csv(self, sender):
        ...
        try:
            # Crea la carpeta 'doc' si no existe todavía
